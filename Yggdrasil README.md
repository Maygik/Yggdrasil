# Yggdrasil — Source Engine Model Porting Tool

Yggdrasil is a project-driven tool for importing, configuring, and exporting 3D models for **Source Engine games**, with **Garry’s Mod (GMod)** as the primary target.
...
```
Yggdrasil.Domain         → Pure data + validation
Yggdrasil.Application    → Pipelines + use-cases
Yggdrasil.Infrastructure → Concrete implementations (Assimp, QC, SMD, IO)
Yggdrasil.Cli            → Headless runner (Phase 1)
Yggdrasil.Presentation   → WinUI editor (Phase 2+)
Yggdrasil.Rendering      → Vortice viewport (Phase 3+)
```
